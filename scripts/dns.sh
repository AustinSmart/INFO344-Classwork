...
# Get all
if ([ -z "$1" ]) then
dns=$(curl -X GET "https://api.cloudflare.com/client/v4/zones/$zone/dns_records?page=1&per_page=20&order=type&direction=desc&match=all" \
     -H "X-Auth-Email: 4.smart.austin@gmail.com" \
     -H "X-Auth-Key: $CFTOKEN" \
     -H "Content-Type: application/json" | jq -r .result)
else
# Get type
dns=$(curl -X GET "https://api.cloudflare.com/client/v4/zones/$zone/dns_records?type=$1&page=1&per_page=20&order=type&direction=desc&match=all" \
     -H "X-Auth-Email: 4.smart.austin@gmail.com" \
     -H "X-Auth-Key: $CFTOKEN" \
     -H "Content-Type: application/json" | jq -r .result)
fi
